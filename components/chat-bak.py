...
def submit_message():
    if st.session_state.text_input_box:
        ...
        # Reset the last voice prompt so the user can record the exact same thing again if needed
...
def render_chat(supabase, gemini_client, chat_model_name, embedding_model_name):
    ...
    with input_container:
        ...
        with col1:
            # The text input box is tied to st.session_state.text_input_box
            st.text_input(
                "Type your message...", 
                key="text_input_box", 
        ...
        with col2:
            ...
            # If a new voice transcription is received, populate the text box and rerun
    ...
    prompt = None
    # Now we ONLY check for submitted_prompt (which requires pressing Enter)
